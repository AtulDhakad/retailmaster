import { IMultiSelectOption } from '../../models/multiselectoption';
import { IMultiSelectSettings } from '../../models/multiselectsettings';
import { IMultiSelectTexts } from '../../models/multiselecttexts';
import {
  NgModule,
  Component,
  Pipe,
    ...
    <div class="dropdown">
      ...
      <ul *ngIf="isVisible" class="dropdown-menu" [class.pull-right]="settings.pullRight" [class.dropdown-menu-right]="settings.pullRight"
        ...
        <li class="dropdown-item" [ngStyle]="getItemStyle(option)" *ngFor="let option of options | searchFilter:searchFilterText"
          ...
          <a *ngIf="!option.isLabel" href="javascript:;" role="menuitem" tabindex="-1" [style.padding-left]="this.parents.length>0&&
                           this.parents.indexOf(option.id)<0&&'30px'">
  ...
  setSelected(_event: Event, option: IMultiSelectOption) {
    ...
    if (index > -1) {
      ...
      if (parentIndex >= 0) {
        ...
        const childIds = this.options.filter(child => this.model.indexOf(child.id) > -1
          && child.parentId === option.id).map(child => child.id);
